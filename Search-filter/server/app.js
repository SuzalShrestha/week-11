...
app.get('/musicians', async (req, res, next) => {
    ...
    // Your code here
    if(req.query.firstName)query.where.firstName = {
    ...
    // Your code here
    if(req.query.lastName)query.where.lastName = {
...
if(req.query.musicianFields){
    if(req.query.musicianFields[0]==="all"){
        ...
        query.attributes = {exclude:["firstName","lastName","createdAt","updatedAt"]};
    }else if(req.query.musicianFields){
   ...
   if(req.query.bandFields){
    if(req.query.bandFields[0]==="all"){
        query.include[0].attributes = {exclude:[]};
    }
    else if(req.query.bandFields[0]==="none"){
        query.include[0].attributes = {exclude:["name","createdAt","updatedAt"]};
    }else if(req.query.bandFields){
        query.include[0].attributes = req.query.bandFields;
    }
   }
   if(req.query.instrumentFields){
    if(req.query.instrumentFields[0]==="all"){
        query.include[1].attributes = {exclude:[]};
    }
    else if(req.query.instrumentFields[0]==="none"){
        query.include[1].attributes = {exclude:["type","createdAt","updatedAt"]};
    }else if(req.query.instrumentFields){
    ...
    // Your code here
    if(req.query.order){
        query.order = req.query.order.map((order)=>{
            if(order.includes(",")){
                return order.split(",");
            }else{
                return [order];
